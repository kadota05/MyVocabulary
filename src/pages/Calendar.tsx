import { ChevronLeft, Grid2x2, Move, Plus, Search, Trash2 } from "lucide-react";
...
const isWithinNoExpandZone = (target: EventTarget | null) =>
  target instanceof HTMLElement &&
  Boolean(target.closest('[data-no-expand="true"]'));

const isWithinKeepExpandedZone = (target: EventTarget | null) =>
  target instanceof HTMLElement &&
  Boolean(target.closest('[data-keep-expanded="true"]'));

export default function Calendar() {
  const navigate = useNavigate();
  const { events, updateEvent, moveEvent, deleteEvent, loadEvents } = useCalendarStore(
    (state) => ({
      events: state.events,
  ...
  const editingFieldRef = useRef<HTMLInputElement | HTMLTextAreaElement | null>(
    null,
  );
  ...
  const showCurrentTime =
    isTodayView && nowMinutes >= 0 && nowMinutes <= TOTAL_MINUTES;
  ...
  useEffect(() => {
    if (
      expandedEventId &&
      !events.some((event) => event.id === expandedEventId)
    ) {
  ...
  useEffect(() => {
    if (!expandedEventId || editing || dragPreview) return;
    ...
    const handleMouseDownOutside = (event: MouseEvent) => {
      ...
      // カード内の要素かどうかを確認
      const isInsideCard = target.closest('.calendar-event');
      if (!isInsideCard) {
        setExpandedEventId(null);
      }
    };
  ...
  }, [expandedEventId, editing, dragPreview]);
  ...
  const timelineEvents = useMemo(() => {
    ...
    if (dragPreview) {
      const index = scoped.findIndex(
        (event) => event.id === dragPreview.eventId,
      );
      if (index !== -1) {
        scoped[index] = {
          ...scoped[index],
  ...
  const monthDots = useMemo(() => {
    return Object.entries(eventsByDate).reduce<Record<string, number>>(
      (acc, [key, value]) => {
  ...
  const startDragFromOrigin = () => {
    ...
    setExpandedEventId((prev) => (prev === origin.eventId ? null : prev));
  ...
  const handleEventPointerDown =
    ...
    (event: React.PointerEvent<HTMLElement>) => {
      ...
      if (
        !isWithinKeepExpandedZone(target) &&
  ...
  const handleEventPointerCancel = (event: React.PointerEvent<HTMLElement>) => {
    if (
      pressOriginRef.current &&
      pressOriginRef.current.pointerId === event.pointerId
    ) {
      clearPressTimer();
      pressOriginRef.current = null;
    }
  ...
  const handleDeleteEvent = (eventId: string) => {
    ...
    setEditing((prev) => (prev?.id === eventId ? null : prev));
    setEditingValue("");
    setDragPreview((prev) => (prev?.eventId === eventId ? null : prev));
  };
  ...
  const commitEditing = () => {
    if (!editing) return;
    const value =
      editing.field === "title" ? editingValue.trim() : editingValue.trim();
      ...
      <div className="calendar-day-frame">
        <header className={classNames(
          ...
          <div className="calendar-day-toolbar__actions">
            <button
              className="calendar-day-toolbar__action"
              type="button"
              aria-label="一覧に切り替え"
              onClick={showComingSoon}
            >
            ...
            <button
              className="calendar-day-toolbar__action is-accent"
              type="button"
              aria-label="予定を追加"
        ...
        >
          {weekDates.map((date) => {
            ...
            return (
              ...
              >
                <span
                  className="calendar-day-week-grid__weekday"
                  aria-hidden="true"
                >
                  {weekdayLabel}
                </span>
                <span className="calendar-day-week-grid__number">
                  {date.getDate()}
                </span>
                {hasEvents && (
                  <span
                    className="calendar-day-week-grid__dot"
                    aria-hidden="true"
                  />
                )}
              </button>
          ...
          <div className="calendar-timeline calendar-timeline--day">
            ...
            >
              ...
              {timelineEvents.map((event) => {
                ...
                const isDimmed =
                  expandedEventId !== null && expandedEventId !== event.id;
                const titleEditing =
                  editing?.id === event.id && editing.field === "title";
                const memoEditing =
                  editing?.id === event.id && editing.field === "memo";
                ...
                return (
                  <article
                    ...
                    className={classNames(
                      "calendar-event",
                      `calendar-event--${event.color ?? "white"}`,
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            if (!isExpanded) {
                              ...
                              startEditingField(event.id, "title", event.title);
                            }
                          }}
                        >
                          {displayTitle}
                        ...
                        <>
                          {memoEditing ? (
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                startEditingField(event.id, "memo", event.memo);
                              }}
                            >
                              {displayMemo}
                            </button>
                      ...
                      >
                        <button
                          ...
                          className={classNames(
                            ...
                            "calendar-event__action--delete",
                            deletingEventId === event.id && "is-deleting"
                          )}
                          data-no-drag="true"
                          data-no-expand="true"
                          aria-label="予定を削除"
                        ...
                        <button
                          type="button"
                          className="calendar-event__action calendar-event__action--move"
                          data-no-expand="true"
                          data-keep-expanded="true"
                          data-no-drag="true"
                          aria-label="予定を移動"
                          onPointerDown={(e) => {
                            ...
                            const duration = Math.max(calendarEvent.end - calendarEvent.start, MIN_EVENT_DURATION);
                            dragStateRef.current = {
