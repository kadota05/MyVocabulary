...
export default function Words(){
  ...
  async function saveEdit(){
    ...
    try{
      ...
      if (msg === 'DUPLICATE_PHRASE') alert('同じ Phrase（trim/小文字比較）で既に登録済みです')
      else if (msg === 'VALIDATION_EMPTY_PHRASE') alert('Phrase は必須です')
  ...
  return (
    <div className='words-screen'>
      <header className='words-header'>
        <div className='words-header__panel'>
          ...
          <input
            className='input'
            placeholder='検索（phrase / meaning / example / source）'
            value={q}
            onChange={e=> setQ(e.target.value)}
          />
          <div className='words-header__metrics'>
            合計 {items.length} 件（表示 {filtered.length} 件）
          </div>
        </div>
      </header>
      ...
      <main className='words-main'>
        {loading ? (
          ...
          <div className='card center' style={{ minHeight:120 }}>見つかりません</div>
        ) : (
              ...
              <div key={w.id} className='card'>
                {editId === w.id ? (
                  <div className='col' style={{ gap:8 }}>
                    ...
                    <label className='label'>Meaning（任意）</label>
